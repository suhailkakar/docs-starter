...
On the TON chain, the proxy app receives your TON transaction and prepares it for the TAC Adapter (on EVM side). When you interact with an EVM application, the TON proxy:
...
5. Sends results back through the TAC Adapter
...
You send a transaction using your TON wallet to TAC Adapter. The proxy formats your request and prepares any assets for transfer through the sequencer network.
</Step>
...
When sending assets from TON to EVM, the TON proxy locks them in the TAC Adapter. This ensures assets can't be double-spent and maintains the correct token supply across chains.
