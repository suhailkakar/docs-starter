...
The network's security on both the TAC EVM Layer and TAC Adaptor relies on real economic stake in the system. These groups of validators (EVM) & sequencers (Adaptor) must stake collateral and actively participate in validation rounds. Good performance earns rewards, while incorrect operations face penalties. This economic model aligns incentives across the network, ensuring participants work to maintain security and efficiency.
...
The process begins when a user connects their TON wallet to interact with an EVM application. The TAC Proxy Contract deployed on TON handles this interaction by locking the user's assets in the adaptor wallet and generating a message. As soon as this happens, sequencers begin monitoring the transaction.
</Step>
...
TAC structures its transaction processing using epochs, a time-based system that ensures orderly processing of messages through the TAC Adaptor. Each epoch is calculated using the formula:
...
## TAC Adaptor Security

TAC uses a combination of message validation and transaction protection to ensure the security of transactions through the TAC Adaptor:
...
### TAC Adaptor Consensus Architecture
...
### TAC Adaptor Economic Framework
...
TAC creates value for multiple stakeholders in the blockchain ecosystem. Users gain the ability to use their familiar TON wallet while accessing the vast world of EVM applications deployed on the TAC EVM Layer, without dealing with complex routing processes. Developers benefit from easy integration paths and automatic proxy generation, backed by secure message routing and a comprehensive SDK.
