...
The network's security on both the TAC EVM Layer and TAC Adapter relies on real economic stake in the system. These groups of validators (EVM) & sequencers (Adapter) must stake collateral and actively participate in validation rounds. Good performance earns rewards, while incorrect operations face penalties. This economic model aligns incentives across the network, ensuring participants work to maintain security and efficiency.
...
TAC structures its transaction processing using epochs, a time-based system that ensures orderly processing of messages through the TAC Adapter. Each epoch is calculated using the formula:
...
## TAC Adapter Security

TAC uses a combination of message validation and transaction protection to ensure the security of transactions through the TAC Adapter:
...
### TAC Adapter Consensus Architecture
...
### TAC Adapter Economic Framework
