...
description: Deep dive into TAC's sequencer network - how it maintains secure communication through the TAC Adapter
---
...
Sequencers are nodes that form transaction trees, with one tree for TON and another for EVM. They use Merkle trees to organize messages in the TAC Adapter, collecting log messages from TON and events from the EVM side within specific time intervals set by the network.

A sequencer takes data from smart contracts, part of the execution layer, and aggregates transactions received over time T (a global network parameter). This organized approach ensures orderly processing of all messages through the TAC Adapter.
...
- At the TAC Adapter level, groups must achieve network-wide consensus. The tree is only applied for operations when enough groups submit identical trees, creating multiple layers of security.
...
The sequencer network ensures transaction integrity through comprehensive validation. When processing messages through the TAC Adapter, sequencers verify that:
