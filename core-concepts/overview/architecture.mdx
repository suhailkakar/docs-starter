...
TAC's architecture solves cross-chain interaction through three interconnected layers: a full EVM blockchain, a secure messaging network, and intelligent proxy contracts. Together, they create an environment where TON users can interact with EVM applications as if they were native TON apps.
...
<Card title="TON Adapter" icon="arrow-left-right">
  Decentralized sequencer network that securely routes messages between TON and
  EVM.
</Card>
...
<Step title="Sequencer Groups" icon="users">
  Sequencers organize into groups that must reach 66% internal consensus. Each
...
<Step title="Detection (TON Adapter)" icon="radar">
  **Sequencers monitor and collect:** - Sequencers detect new events on TON -
  Transaction details stored in local databases - Asset transfers validated
  against message parameters
</Step>

<Step title="Consensus (TON Adapter)" icon="handshake">
  **Multi-layer validation process:** - Individual sequencers form Merkle trees
  - Groups reach 66% internal consensus - Cross-group validation ensures
  networkagreement - Cryptographic proofs generated for execution
</Step>

<Step title="Execution (TAC EVM)" icon="settings">
  **Target chain processes transaction:** - EVM proxy receives validated message
  - Assets are minted or unlocked as needed - Target contract methods executed
  with parameters - Results and state changes recorded on-chain
</Step>
