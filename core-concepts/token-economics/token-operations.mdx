...
Understanding token operations provides the foundation for exploring specific token types and their unique characteristics in the TAC ecosystem.

<CardGroup cols={2}>
  <Card
    title="Native Tokens"
    icon="star"
    href="/core-concepts/token-economics/native-tokens"
  >
    Learn about native TON and TAC token handling
  </Card>
  <Card
    title="Bridged Tokens"
    icon="bridge"
    href="/core-concepts/token-economics/bridged-tokens"
  >
    Explore how external tokens are integrated into TAC
  </Card>
</CardGroup>
